  ...
  const elements = useEditorEngine(engine, (s: EditorState) => s.elements);
    ...
    useLayoutEffect(() => {
      const nextTargets = selectedIds
        .filter((id: string) => id !== interaction.editingId)
        .map((id: string) => document.querySelector(`[data-element-id="${id}"]`) as HTMLElement)
        .filter(Boolean);
    ...
    const elementGuidelines = useMemo(() => {
      const selectedElements = selectedIds.map((id: string) => elements.find((el: Element) => el.id === id)).filter(Boolean);
      const parentIds = new Set(selectedElements.map((el) => el!.parentId).filter((id): id is string => id !== undefined));
      ...
      return elements
        ...
        .map((el: Element) => document.querySelector(`[data-element-id="${el.id}"]`) as HTMLElement)
        .filter(Boolean);
